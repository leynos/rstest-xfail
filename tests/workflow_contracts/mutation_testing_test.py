...
rstest-xfail's caller is declarative configuration. These tests parse
the caller with PyYAML and assert the contract it must uphold: the
caller references the correct reusable workflow at a commit SHA, and
drift such as repointing at a branch, widening permissions, or losing
the feature-args and linker-setup configuration fails CI on the pull
request rather than surfacing in a scheduled or manual run. Dependabot
owns the pinned SHA value; these tests do not assert which SHA is
pinned, only that one is.
...
def test_triggers_keep_schedule_and_plain_dispatch() -> None:
    ...
    assert schedule == [{"cron": "35 11 * * *"}], (
        f"on.schedule must be the daily 11:35 UTC cron, got {schedule!r}"
    )
...
def test_with_block_carries_the_caller_configuration() -> None:
    """The caller mirrors the CI baseline: features and linker setup only."""
